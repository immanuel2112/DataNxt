...
from imichael.wipro.datanxt.model.config_models import Page
...
class Home(ApplicationWindowTemplate):
    ...
    def build_display_frame(self):
        ...
        if self.page.events is not None:
            ...
            refresh = ttk.Button(config_display_title_events, text="Refresh", command=self.show_sys_model)
            refresh.grid(column=0, row=0, padx=2)
            add = ttk.Button(config_display_title_events, text="Add")
            add.grid(column=1, row=0, padx=2)
            config_display_title_events.pack(fill=X)

        # Fetch table data and build tree view with the data retrieved
        # config_display_parent_window = ttk.Frame(self.config_display_window, style='Content.TFrame', borderwidth=2)
        parent_data = get_table(self.sessiondetails, self.page.table)
        ...
        if self.page.child_table is not None:
            ...
            self.config_display_child_window = ttk.Frame(self.config_display_window, style='Content.TFrame', borderwidth=2)
        ...
        # config_display_parent_window.pack(fill=BOTH, expand=1)
    ...
    def display_child_frame(self, event):
        ...
        # Build title for display window
        config_display_child_title_window = ttk.Frame(self.config_display_child_window, style='Header.TFrame', borderwidth=2)
        ...
        if self.child_page.events is not None:
            config_display_child_title_events = ttk.Frame(self.config_display_child_window, style='Header.TFrame', borderwidth=2)
            refresh = ttk.Button(config_display_child_title_events, text="Refresh", command=self.display_child_frame)
            refresh.grid(column=0, row=0, padx=2)
            add = ttk.Button(config_display_child_title_events, text="Add")
            add.grid(column=1, row=0, padx=2)
        ...
        child_table = get_table(self.sessiondetails, self.child_page.table)
        ...
        # Pack the contents
        self.display_child_table_tree.pack(side=TOP, fill=X)
        # config_display_child_window.pack(fill=BOTH, expand=1)
